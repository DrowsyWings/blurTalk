    ...
    if (handleValidation()) {
      try {
        ...
        if (response.data.statusCode === 200) {
          ...
          localStorage.setItem("blur-talk-user", JSON.stringify(user));
  ...
  return (
    <div className="bg-[#131324] px-6 sm:px-0 w-full h-[100vh] flex flex-col justify-center gap-[1rem] items-center">
      <form
        ...
        className="bg-[#0b0b11] flex flex-col gap-[2rem] rounded-[2rem] px-[3rem] sm:px-[5rem] py-[2rem] sm:py-[3rem]"
      >
        <div className="flex items-center justify-center">
          <img
            src={logo}
            alt="logo"
            className="h-[4rem] sm:h-[6rem] rounded-full"
          />
        ...
        <div className="flex flex-col gap-1">
          ...
          <Link to="/account-recovery">
            <span className="text-white hover:text-[#4e0eff] cursor-pointer">
              Forgot Password?
            </span>
          </Link>
        </div>
        <button
          type="submit"
          className="bg-[#4e0eff] text-white px-[3rem] py-[1rem] font-bold cursor-pointer rounded-[0.4rem] text-[1rem] hover:bg-[#4e0eff]"
        >
          LOGIN
        </button>
